...
app.post('/api/score', _scoreRateLimit, async (req, res) => {
  ...
  try {
    ...
    // Fire PageSpeed immediately — runs in parallel with HTML fetch and AI

    const psPromise = _fetchPageSpeed(url);



    // Fetch HTML first (required for AI analysis)

    const htmlResult = await _fetchUrlHtml(url);
    ...
    // Start AI immediately — don't wait for PageSpeed (decoupled from critical path)

    const aiPromise = _aiAnalyze(url, htmlResult.html, checks, { mobileScore: null, lcp: null, fcp: null, error: null });



    // Wait for AI, then check if PageSpeed is already done (zero added wait if resolved)

    let analysis = await aiPromise;

    const psResult = await Promise.race([

      psPromise,

      new Promise(r => setImmediate(() => r({ mobileScore: null, lcp: null, fcp: null, error: 'not-ready' }))),
