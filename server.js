...
// ── RATE LIMITER — checkout sessions ─────────────────────────────────────────
// Max 5 checkout session requests per IP per 10 minutes.
// Single-instance safe (Railway single dyno). Guards against carding bots.
const _checkoutRateBuckets = new Map();
setInterval(() => {
  const cutoff = Date.now() - 10 * 60 * 1000;
  for (const [ip, entry] of _checkoutRateBuckets) {
    if (entry.windowStart < cutoff) _checkoutRateBuckets.delete(ip);
  }
}, 60000);

function _checkoutRateLimit(req, res, next) {
  const ip = (req.headers['x-forwarded-for'] || '').split(',')[0].trim() || req.socket.remoteAddress || 'unknown';
  const now = Date.now();
  const WINDOW = 10 * 60 * 1000;
  const LIMIT = 5;
  const entry = _checkoutRateBuckets.get(ip);
  if (!entry || now - entry.windowStart > WINDOW) {
    _checkoutRateBuckets.set(ip, { windowStart: now, count: 1 });
    return next();
  }
  if (entry.count >= LIMIT) {
    console.warn(`[checkout-rate-limit] blocked ip=${ip} count=${entry.count}`);
...
// ── API: CONFIG (exposes safe public config to frontend) ─────────────────────
...
app.post('/api/create-checkout-session', _checkoutRateLimit, async (req, res) => {

  try {

    const stripe = Stripe(process.env.STRIPE_SECRET_KEY);

    const { plan, withHosting, withCarePlan, cfTurnstileToken } = req.body;

    // Turnstile bot verification — enforced when TURNSTILE_SECRET_KEY env var is set
    if (process.env.TURNSTILE_SECRET_KEY) {
      if (!cfTurnstileToken) {
        return res.status(400).json({ error: 'Bot verification token required.' });
      }
      const remoteip = (req.headers['x-forwarded-for'] || '').split(',')[0].trim() || req.socket.remoteAddress;
      const verifyRes = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ secret: process.env.TURNSTILE_SECRET_KEY, response: cfTurnstileToken, remoteip }),
      });
...
const SCORE_EVENTS_FILE = path.join(SCORE_DATA_DIR, 'score-events.jsonl');
...
function _logScoreEvent(evt) {
  try { fs.appendFileSync(SCORE_EVENTS_FILE, JSON.stringify(evt) + '\n'); } catch (e) { console.error('[score-tool] Event write error:', e.message); }
}

function _estIsoTimestamp() {
  const now = new Date();
  const isDST = new Intl.DateTimeFormat('en-US', { timeZone: 'America/New_York', timeZoneName: 'short' }).format(now).includes('EDT');
  const local = now.toLocaleString('sv-SE', { timeZone: 'America/New_York' });
  return local.replace(' ', 'T') + (isDST ? '-04:00' : '-05:00');
}

function _detectScoreSource(req) {
  const qs = req.query.utm_source || req.query.source || '';
  if (qs) return qs === 'email' || qs === 'email_link' ? 'email_link' : qs;
  const ref = req.headers.referer || '';
  if (!ref) return 'direct';
  try { const h = new URL(ref).hostname; if (h.includes('almadigitalservices.com')) return 'organic'; } catch {}
  return 'organic';
}
...
app.post('/api/score', _scoreRateLimit, async (req, res) => {
  ...
  try {
    ...
    const estIso = _estIsoTimestamp();
    _logScoreEvent({
      event: 'scan_completed',
      timestamp: estIso,
      url: result.url,
      source: _detectScoreSource(req),
    ...
    _logScoreEvent({
      ...
      email_hash: crypto.createHash('sha256').update(email.toLowerCase().trim()).digest('hex'),
    });

    console.log(`[score-tool] ${url} => ${analysis.overallGrade} (${Date.now() - t0}ms) [session=${sessionId}]`);
    res.json(result);
  } catch (err) {
